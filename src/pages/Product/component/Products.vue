<template>
    ...
    <div class="child4">{{ short_des }}</div>
    ...
    methods: { 
      ...
      Delete(i) {
        ...
        if (r == true) {
         document.getElementById(`sp${i}`).classList.toggle('cvs');
          ...
          axios.put(`/api/products/trash/${this.id}`).then(function( response ){
      ...
      restore(i) {
        ...
        document.getElementById(`m${i}`).classList.toggle('cvs');
         ...
         var r = confirm(`Are you sure you want to restore Product id ${i}`);
        if (r == true) {
         document.getElementById(`sp${i}`).classList.toggle('cvs');
  
          axios.put(`/api/products/restoreTrashed/${this.id}`).then(function( response ){
                console.log(response);
                  if(response.data.stateNum == 201 || self.statusnumber == 200){
